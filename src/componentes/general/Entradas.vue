                ...
                <div class="w-full">
                    ...
                    <div class="flex-1 lg:flex items-center space-x-2">
                        ...
                        <div class="ml-0 text-1xl font-bold text-gray-600 ">
                            <p class="text-center lg:flex">Entradas [{{entradas.length}}]</p>
                            <p v-if="entradas.length == 500" class="text-center text-xs text-red-400 font-semibold">Máximo 500 registros por lote, por favor use el paginador</p>
                        </div>

                        <button @click="index_entradas()" class="px-2 font-semibold text-xs cursor-pointer rounded bg-green-600 flex items-center border text-white " title="actualizar">
                        ...
                        <div v-if="buscar_div" class="intro-b h-auto lg:h-6 flex-1 lg:flex items-center">
                            ...
                            <div class="lg:mr-2 mt-4 lg:mt-0">
                                <input placeholder="Producto.." type="text" v-model="buscador_nombre" class="w-full h-6 rounded focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 px-1 border border-gray-300">
                            </div>
                        
                            <div class="lg:mr-2 mt-2 lg:mt-0">
                                <input placeholder="Pedido.." type="text" v-model="buscador_pedido" class="w-full h-6 rounded focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 px-1 border border-gray-300">
                            </div>
                        
                            <div class="lg:mr-2 mt-2 lg:mt-0">
                                <input placeholder="Código.." type="text" v-model="buscador_codigo" class="w-full h-6 rounded focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 px-1 border border-gray-300">
                            </div>
                        
                            <div class="lg:mr-2 mt-2 lg:mt-0">
                                <input placeholder="Proveedor.." type="text" v-model="buscador_proveedor" class="w-full h-6 rounded focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 px-1 border border-gray-300">
                            </div>

                            <div class="lg:mr-2 mt-2 lg:mt-0">
                                <input placeholder="Ubicación.." type="text" v-model="buscador_ubicacion" class="w-full h-6 rounded focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 px-1 border border-gray-300">
                            </div>
                    ...
                    <div class="flex items-center lg:mr-2 mt-3 space-x-2">
                        ...
                        <div v-if="calcular_div" class="intro-r rounded w-full border h-6 px-2 flex items-center space-x-3">
                            ...
                            <p class="text-sm">Entradas #{{totales.entradas}}</p>
                    ...
                    <div class="intro-b mt-3 intro-r h-screen overflow-y-auto color-hueso border shadow-md rounded border-gray-400  pb-2">
                
                        <table v-if="entradas.length" class="">
                            ...
                            <thead class="h-6">
                                ...
                                <th v-if="vista[2].valor" class="hidden md:table-cell h-4 px-1 lg:px-5 py-0 border-b border-gray-300 lowercase text-left sm:lowercase md:lowercase lg:lowercase xl:uppercase text-xs font-semibold lg:font-semibold text-gray-500">
                                ...
                                <th v-if="vista[5].valor" class="hidden md:table-cell h-4 px-1 lg:px-5 py-0 border-b border-gray-300 lowercase text-left sm:lowercase md:lowercase lg:lowercase xl:uppercase text-xs font-semibold text-gray-500">
                                ...
                                <th v-if="vista[7].valor" class="hidden lg:table-cell h-4 px-1 lg:px-5 py-0 border-b border-gray-300 lowercase text-left sm:lowercase md:lowercase lg:lowercase xl:uppercase text-xs font-semibold text-gray-500">
                                    <span class="">&nbsp;</span>
                                </th>
                            ...
                            <tbody>
                                ...
                                <tr :key="key" v-for="(entrada,key) in entradas"  class="bg-white border-gray-300 hover:bg-gray-200">
                                    ...
                                    <td v-if="vista[2].valor" class="hidden md:table-cell py-1 md:text-xs border-b border-gray-300 leading-5 text-xs font-semibold text-gray-500 text-center">
                                      <span class="block">{{entrada.proveedor}}</span>
                                    </td>
                    
                                    <td v-if="vista[3].valor" class="hidden md:table-cell py-1 md:text-xs border-b border-gray-300 leading-5 text-xs font-semibold text-gray-500 text-center">
                                      <span class="" > {{entrada.ubicacion}}</span>
                                    </td>
                    
                                    <td class="table-cell py-1 md:text-xs border-b border-gray-300 text-xs font-semibold text-gray-500 text-center">
                                      <span>{{format_number(entrada.cantidad)}}</span>
                                    </td>
                    
                                    <td v-if="vista[4].valor" class="hidden md:table-cell py-1 md:text-xs border-b border-gray-300 text-gray-500 leading-5 text-xs font-semibold text-center">
                                      <span class="" >${{format_number(entrada.costo_unidad)}}</span>
                                    </td>
                    
                                    <td v-if="vista[5].valor" class="hidden md:table-cell py-1 md:text-xs border-b border-gray-300 leading-5 text-xs font-semibold text-gray-500 text-center">
                                      <span class="" >${{ total_entrada(entrada) }}</span>
                                    </td>
                    
                                    <td v-if="vista[6].valor" class="hidden lg:table-cell py-1 max-w-0 w-full md:text-xs border-b border-gray-300 leading-5 text-xs font-semibold text-gray-500 text-center">
                                      <span class="">{{entrada.entrada }}</span>
                                    </td>
                    
                                    <td v-if="vista[7].valor" class="hidden lg:table-cell py-1 max-w-0 w-full md:text-xs border-b border-gray-300 text-xs font-semibold text-gray-500">
                    
                                        <button @click="eliminar(entrada)" title="eliminar entrada" id="project-options-menu-0" type="button" class="w-8 h-8 px-2 mx-auto text-gray-400 focus:text-gray-500">
                        ...
                        <p v-else class="text-gray-500 font-semibold text-md mt-1 text-center">No hay entradas registradas!</p>
    ...
    export default {
        ...
        data(){
            return{
                ...
                buscador_ubicacion:'',
                buscador_pedido:'',
                vista: [{ 'name':'pedido','valor':true}, { 'name':'código','valor':false}, { 'name':'proveedor','valor':false}, { 'name':'ubicación','valor':true},{ 'name':'v.unitario','valor':false},{ 'name':'total','valor':true},{ 'name':'AMD','valor':false}, { 'name':'eliminar','valor':false}],
                menu_mostrar:false,
                totales:{'entradas':0, 'totales':0},
                calcular_div:true,
                buscar_div:false
        ...
        methods:{
            ...
            ocultar_menu_mostrar(){
                ...
                localStorage.setItem('vista_entradas',JSON.stringify(this.vista))
            },
            ...
            index_entradas(){
                Entrada.index({'year': this.very.num,'month':this.verm.num})
            },
            
            index_entradas_default(){
                ...
                Entrada.index({'year':year,'month': num_months[month]})
            ...
            eliminar(json){ Entrada.delete({'entrada_id':json.id}) },

            set_config(){

                try {
                  this.vista = Funciones.check_config(this.vista,'vista_entradas')
            ...
            calcular(){
                ...
                let entradas_reg = this.entradas

                var entradas = 0 
                var totales = 0

                entradas_reg.map((item,index, array)=>{

                    entradas = entradas+item.cantidad
                    totales = totales+(item.cantidad*item.costo_unidad) 
                
                })

                this.totales.entradas = Funciones.format_tnumber(entradas)
        ...
        computed:{
            ...
            entradas(){
                ...
                let todos = Store.state.entradas
                ...
                var count_proveedor = this.buscador_proveedor.toString()
                ...
                let temp_m = temp_c
                if(count_proveedor.length > 0){	temp_m = Funciones.buscador_texto(count_proveedor,todos,'proveedor') }
        ...
        mounted(){
            this.$nextTick(()=>{
                ...
                Aplicacion.check_login(()=>{
                    if(!Store.state.entradas.length){ this.index_entradas_default() }
                })
        ...
        watch:{
            ...
            verm(){ this.index_entradas() },
            very(){ this.index_entradas() },
